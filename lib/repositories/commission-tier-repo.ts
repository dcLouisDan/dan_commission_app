import {
  CommissionTier,
  CommissionTierInsert,
  CommissionTierUpdate,
} from "../types/commission-tier";
...
export async function getCommissionTiers(): Promise<
  DbResult<CommissionTier[] | null>
> {
  const supabase = await createClient();
  try {
    const { data, error } = await supabase
      .from(COMMISSION_TIER_TABLE)
      .select("*");
    if (error) {
...
export async function getCommissionTierById(
  id: string,
): Promise<DbResult<CommissionTier | null>> {
  const supabase = await createClient();
  try {
    const { data, error } = await supabase
      .from(COMMISSION_TIER_TABLE)
    ...
    if (!data) {
      return {
        ok: false,
        error: { type: "unknown", raw: new Error("No data returned") },
      };
    }
    return { ok: true, data: data as CommissionTier };
  } catch (error) {
...
export async function insertCommissionTier(
  tier: CommissionTierInsert,
): Promise<DbResult<CommissionTier | null>> {
  const supabase = await createClient();
  try {
    const { data, error } = await supabase
